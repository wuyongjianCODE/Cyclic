...
epoch_of_current_iteration=np.array([15,25,5,5,5,1,3,1,2,3,4,4,4,4,4,1,1])
...
class NucleusConfig(Config):
    ...
    STEPS_PER_EPOCH = 30// IMAGES_PER_GPU#(657 - len(VAL_IMAGE_IDS)) // IMAGES_PER_GPU
...
def detect(model, dataset_dir, subset, iter=0):
    ...
    for image_id in dataset.image_ids:
        ...
        mask_result=np.zeros((image.shape[0],image.shape[1]),dtype=np.uint8)
        masks=r['masks']
        if TAKE_5_STEP==True:
            scores=r['scores']
            for i in range(masks.shape[2]):
                ...
                if scores[i]>0.85 and i <40:
                    ...
                    skimage.io.imsave("{}/{}.png".format(os.path.abspath(name_path),dataset.image_info[image_id]["id"]+'_iteration_'+str(iter)+'_mask_'+str(i)),
                                      mask_temp)
        for i in range(masks.shape[2]):
            mask_result[masks[:,:,i]==1]=255
