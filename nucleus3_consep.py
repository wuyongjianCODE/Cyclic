...
epoch_of_current_iteration=np.array([25,5,5,5,5,1,3,1,2,3,4,4,4,4,4,1,1])
...
RESULTS_DIR = os.path.join(ROOT_DIR, "results/consepcrop/")
TAKE_5_STEP=False
...
class NucleusConfig(Config):
    ...
    STEPS_PER_EPOCH = 50// IMAGES_PER_GPU#(657 - len(VAL_IMAGE_IDS)) // IMAGES_PER_GPU
...
class NucleusDataset(utils.Dataset):
    ...
    def load_nucleus(self, dataset_dir, subset):
        ...
        self.add_class("consep", 1, "consep1")
        # Which subset?
        ...
        for image_id in image_ids:
            if os.path.exists(os.path.join(dataset_dir+'/{}/'.format(subset), "Instance_mask/{}".format(image_id[:-4]))):
                self.add_image(
                    "consep",
                    image_id=image_id,
...
# test.test(submit_dir,r'D:\BaiduNetdiskDownload\BrestCancer\MoNuSAC-master\MoNuSAC_mask')
    dice=testcc.test_XMetric(submit_dir,r'/data1/wyj/M/datasets/consepcrop/Test/Colormask/')
