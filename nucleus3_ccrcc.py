...
epoch_of_current_iteration=np.array([15,300,5,5,5,1,3,1,2,3,4,4,4,4,4,1,1])
...
RESULTS_DIR = os.path.join(ROOT_DIR, "results/ccrcc/")
TAKE_5_STEP=False
...
class NucleusConfig(Config):
    ...
    NUM_CLASSES = 4 + 1  # Background + nucleus
    # Number of training and validation steps per epoch
    STEPS_PER_EPOCH = 700// IMAGES_PER_GPU#(657 - len(VAL_IMAGE_IDS)) // IMAGES_PER_GPU
...
class NucleusDataset(utils.Dataset):
    ...
    def load_nucleus(self, dataset_dir, subset):
        ...
        self.add_class("ccrcc", 2, "ccrcc2")
        self.add_class("ccrcc", 3, "ccrcc3")
        self.add_class("ccrcc", 4, "ccrcc4")
        # Which subset?
...
def detect(model, dataset_dir, subset, iter=0):
    ...
    for image_id in dataset.image_ids:
        ...
        mask_path=path.replace('Images','Mask')
...
# test.test(submit_dir,r'D:\BaiduNetdiskDownload\BrestCancer\MoNuSAC-master\MoNuSAC_mask')
    dice=testcc.test_XMetric(submit_dir)
